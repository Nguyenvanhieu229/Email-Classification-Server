...
class ServerThongMinh:

    def analys(self, vector, model, test_file):
    ...
    def train_model(self, train_data, test_data):
        ...
        os.remove(vector_name)

        return json.dumps(result)

    def chiaTap(self, train_file):
        # try:
        ...
        print(len(mau))
        nhan = [0, [], [], [], [], [], []]
        label = [0, '_label_học_tập', '_label_đơn_hàng', '_label_quảng_cáo', '_label_cảnh_báo', '_label_ngân_hàng',
                 '_label_thư_rác']
    ...
    def daoTao(self, train_file):
        train_data, test_data = self.chiaTap(train_file)
        result = self.train_model(train_data, test_data)
        return result

    def layMoHinhDangSuDung(self):
    ...
    def duDoan(self, data):
        ...
        try:
            model, vector = self.layMoHinhDangSuDung()
        except Exception as e:
            print(e)
    ...
    def tien_xu_ly_mau(self, sample):
        sample = xu_ly_mau.remove_html(sample)
        sample = xu_ly_mau.convert_unicode(sample)
        sample = xu_ly_mau.word_tokenize(sample, format="text")
        sample = sample.lower()
